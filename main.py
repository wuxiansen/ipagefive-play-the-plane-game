...
pygame.display.set_caption("Play the plane game")
...
def main():
    ...
    while running:
        ...
        # ====================定义难度递进操作====================
        if level == 1 and score >= 0:  # 如果达到第二难度等级，则增加3架小型敌机，2架中型敌机，1架大型敌机,并提升小型敌机速度
            level = 2
            level_up_sound.play()
            add_small_enemies(small_enemies, enemies, 3)
            add_mid_enemies(mid_enemies, enemies, 2)
            add_big_enemies(big_enemies, enemies, 1)
            inc_speed(small_enemies, 6)
        elif level == 2 and score > 3000:  # 如果达到第三难度等级
            level = 3
            level_up_sound.play()
            add_small_enemies(small_enemies, enemies, 3)
            add_mid_enemies(mid_enemies, enemies, 2)
        ...
        elif level == 3 and score > 6000:  # 如果达到第四难度等级
            level = 4
            level_up_sound.play()
            add_small_enemies(small_enemies, enemies, 3)
            add_mid_enemies(mid_enemies, enemies, 2)
...
if __name__ == '__main__':
    try:
        logging.debug("starting play the plane game ...")
        main()
    except SystemExit:
        pass
    except:
